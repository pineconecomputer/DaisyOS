...
#ifndef INCLUDE_ZMACHINE_ZM_TYPES_H_
#define INCLUDE_ZMACHINE_ZM_TYPES_H_
...
#define DEFAULT_ROWS 25         /* DaisyVideo text rows */
#define DEFAULT_COLS 40         /* DaisyVideo text columns */
...
#endif
*/
/* RANDOM_FUNC and SRANDOM_FUNC come from zm_port.h. */
...
/* Data access.
 *
 * Upstream jzip defines these as direct subscripts of datap, which is safe
 * there only because load_cache() makes the resident area large enough to
 * cover the dictionary as well as dynamic memory. This port keeps just the
 * writable region resident (11,859 bytes for Zork I rather than 20,480), so
 * an access above zm_dyn_size has to come from the flash story image instead.
 * The dictionary sits at 15137..20023 in Zork I and is read through these
 * accessors, so getting this wrong reads past the end of datap.
 *
 * Writes above zm_dyn_size are a game bug; the Z-machine spec forbids writing
 * to static memory. ZmWriteGuard reports it rather than corrupting flash. */

extern unsigned long zm_dyn_size;
void ZmWriteGuard(unsigned long offset);

static inline zbyte_t get_byte(unsigned long offset) {
  return (offset < zm_dyn_size) ? datap[offset] : ZmStoryByteChecked(offset);
}

static inline zword_t get_word(unsigned long offset) {
  return (zword_t)(((zword_t)get_byte(offset) << 8) + (zword_t)get_byte(offset + 1));
}

static inline void set_byte(unsigned long offset, zword_t value) {
  if (offset >= zm_dyn_size) {
    ZmWriteGuard(offset);
    return;
  }
...
static inline void set_word(unsigned long offset, zword_t value) {
  set_byte(offset, (zword_t)((zword_t)value >> 8));
  set_byte(offset + 1, (zword_t)((zword_t)value & 0xff));
}
...
struct ZmSaveStream;
int save_quetzal( struct ZmSaveStream *, struct ZmSaveStream * );
int restore_quetzal( struct ZmSaveStream *, struct ZmSaveStream * );
...
/* A2Z's mcurses colour attributes and LED helper are not used here;
 * DaisyVideo is monochrome and errors unwind rather than blinking. */
