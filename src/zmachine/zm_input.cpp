...
void z_sread_aread( int argc, zword_t * argv )
{
   ...
   if ( !ZmDynamicRange( argv[0], 2 ) )
   {
      fatal( "z_sread_aread(): text buffer outside dynamic memory" );
      return;
   }
...
static void tokenise_line( zword_t char_buf, zword_t token_buf, zword_t dictionary, zword_t flag )
{
   ...
   /* char_buf and token_buf come from the story. The specification puts them
    * in dynamic memory, but a corrupt or hostile file could point them into
    * static memory, which is no longer part of datap in this port. */
   if ( !ZmDynamicRange( char_buf, 2 ) || !ZmDynamicRange( token_buf, 2 ) )
   {
      fatal( "tokenise_line(): parse buffer outside dynamic memory" );
      return;
   }
