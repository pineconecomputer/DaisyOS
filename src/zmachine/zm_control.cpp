...
void z_restart( void )
{
   unsigned int i, j, scripting_flag;
   ...
   /* Upstream seeds from time(); the SAM3X has no wall clock, and ZmMillis
    * is what both build targets agree on. */
   ...
   /* Reload the writeable data area.
    *
    * Upstream reloads ( h_restart_size / PAGE_SIZE ) + 1 whole pages into
    * datap, which is safe there only because datap runs to the base of high
    * memory. This port sizes datap to h_restart_size exactly, so for Zork I
    * that loop would write 12,288 bytes into an 11,859 byte buffer. Copy the
    * dynamic region from the flash image instead. */
