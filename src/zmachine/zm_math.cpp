...
/* A2Z supplied its own rand()/srand() here because the AVR core has none.
 * This port routes RANDOM_FUNC and SRANDOM_FUNC through zm_port.h to the
 * generator in zm_osdep.cpp instead, so that the firmware and the test
 * harness agree; the replacements are gone because the name srandom also
 * collides with newlib's on the ARM toolchain. */
