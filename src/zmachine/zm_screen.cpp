...
void z_show_status( void )
{
   ...
   if ( get_byte( H_CONFIG ) & CONFIG_TIME )
   {
      ...
      pad_line( ( screen_cols >= 72 ) ? screen_cols - 21 : screen_cols - 16 );
      status_part[count] = &status_line[status_pos];
      write_string( " Time: " );
   ...
   else
   {
      ...
      /* Field positions.
       *
       * Upstream hardcodes an 80 column layout, putting Score 31 columns from
       * the right and Moves 15. On Daisy's 40 column screen that lands Score
       * at column 9, and because pad_line rewinds rather than advancing when
       * the room name has already passed that point, the name is chopped to
       * eight characters and the fields collide: "North of House" came out as
       * "North of Score  0". Narrow screens get abbreviated labels and enough
       * room for a name instead. */
      pad_line( ( screen_cols >= 72 ) ? screen_cols - 31 : screen_cols - 14 );
      status_part[count] = &status_line[status_pos];
      write_string( ( screen_cols >= 72 ) ? " Score: " : " S:" );
      z_print_num( load_variable( 17 ) );
      end_of_string[count++] = status_pos;
      status_line[status_pos++] = '\0';

      pad_line( ( screen_cols >= 72 ) ? screen_cols - 15 : screen_cols - 8 );
      status_part[count] = &status_line[status_pos];
      write_string( ( screen_cols >= 72 ) ? " Moves: " : " M:" );
   ...
   if ( outputting == ON )
   {
      ...
      if ( formatting == ON && screen_window == TEXT_WINDOW )
      {
         ...
         if ( ++lines_written >= ( ( screen_rows - top_margin ) - status_size - 1 ) )
         {
            ...
            {
               get_cursor_position( &row, &col );
               /* min() is an Arduino macro and is not available to the
                * host build; clamp explicitly. */
               if ( row > screen_rows )
                  row = screen_rows;
               ...
               /* Erase the prompt once it has been acknowledged. Upstream
                * steps the cursor to the previous row and back, relying on
                * the story's next output to overwrite the text. When that
                * output happens to be a newline the prompt survives and
                * scrolls up into the transcript, which is what put a stray
                * [MORE] in the middle of Zork III's opening. */
               move_cursor( row, col );
               clear_line(  );
            }
         }
